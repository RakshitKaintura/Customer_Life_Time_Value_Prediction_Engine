  ...
  return (
    <div className="flex items-center gap-1.5 text-xs">
      ...
      <span className={passed ? "text-green-700" : "text-red-600"}>
        {label}: {passed ? "✓" : "✗"} target {higherIsBetter ? "≥" : "≤"} {target}
      </span>
    </div>
  );
}
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <StatCard
          ...
          format="text"
          subtitle="Target: > 0.65"
        />
        <StatCard
          title="Top Decile Lift"
          value={`${lift.toFixed(2)}×`}
          format="text"
          subtitle="Target: > 3.0×"
        />
        <StatCard
          title="MAE % of Mean LTV"
          value={`${(mae_pct * 100).toFixed(1)}%`}
          format="text"
          subtitle="Target: < 15%"
        />
        <StatCard
          title="Calibration Error"
          value={calib.toFixed(4)}
          format="text"
          subtitle="Target: < 0.10"
        />
        <StatCard
          title="BG/NBD R²"
          value={bgnbd_r2.toFixed(4)}
          format="text"
      ...
      <div className="rounded-lg border border-slate-200 bg-white px-5 py-4">
        <p className="mb-3 text-sm font-semibold text-slate-700">Model Target Checklist</p>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <MetricTarget value={bgnbd_r2} target={0.85} label="BG/NBD R²"    higherIsBetter />
        </div>
      </div>
    </div>
  );
