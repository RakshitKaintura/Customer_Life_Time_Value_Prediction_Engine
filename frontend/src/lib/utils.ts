...
export function formatCurrency(value: number | string | null | undefined): string {
  ...
  if (numericValue >= 1_000_000) return `${CURRENCY_SYMBOL}${(numericValue / 1_000_000).toFixed(1)}M`;
  if (numericValue >= 1_000) return `${CURRENCY_SYMBOL}${(numericValue / 1_000).toFixed(1)}K`;
  return `${CURRENCY_SYMBOL}${numericValue.toFixed(0)}`;
}
